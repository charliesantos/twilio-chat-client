...
const SERVER_URI = 'http://127.0.0.1:3030';
...
function QuickChatClient(displayName, conversationId) {
  ...
  const onMessage = (message) => {
    ...
    try {
      ...
      console.warn(`Received invalid json string: ${message.utf8Data}`);
    }
  ...
  const handleEvent = (msgObj) => {
    switch (msgObj.name) {
      case 'new-message':
        this.emit('messageAdded', decodeURIComponent(msgObj.data));
        break;
      case 'member-joined':
        this.emit('memberJoined', msgObj.data);
        break;
      case 'member-left':
        this.emit('memberLeft', msgObj.data);
        break;
      default:
        // Handle other events
  ...
  const sendObject = (obj) => {
    ...
    // Send it off
    this.connection.sendUTF(JSON.stringify(reqObj));
...
QuickChatClient.prototype.getMessages = function getMessages() {
  return this.sendObject({ command: 'get-messages' })
